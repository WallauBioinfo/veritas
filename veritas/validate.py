...
def process_gsalign_vcf(gsalign_vcf, sample_name, output_vcf):
    ...
    Calls fix_gsalign_vcf() first to patch the raw GSAlign output (adds the
    wildcard FILTER definition and strips END= from INFO), then injects GT
    FORMAT/sample columns required by RTG vcfeval.

    Args:
        gsalign_vcf: Path to VCF generated by GSAlign
        sample_name: Sample name to use in the output VCF
        output_vcf: Path to output processed VCF (uncompressed; .gz is returned)

    Returns:
        Path to the compressed VCF file (output_vcf + ".gz")
    """
    fixed_vcf = output_vcf + ".fixed.vcf"
    fix_gsalign_vcf(gsalign_vcf, fixed_vcf)

    with open(fixed_vcf, "r") as fin, open(output_vcf, "w") as fout:
        for line in fin:
            if line.startswith("##"):
                fout.write(line)
...
def compare(
    ...
    tmp_files = [
        f"{rtg_dir}/tmp_tp.vcf.gz",
        f"{rtg_dir}/tmp_tp.vcf.gz.tbi",
        f"{rtg_dir}/tmp_fp.vcf.gz",
        f"{rtg_dir}/tmp_fp.vcf.gz.tbi",
        f"{rtg_dir}/tmp_fn.vcf.gz",
        f"{rtg_dir}/tmp_fn.vcf.gz.tbi",
...
def get_bed_intervals(bed_file):
    ...
    Note: BED format is 0-based half-open [start, end). We store the raw
    integers and compare with an inclusive check (start <= pos <= end) because
    pysam VariantRecord.pos is 1-based. Variants exactly at a region boundary
    are counted as inside the region.
